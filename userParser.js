...
async function load() {
    ...
    for (let i = 0; i < 500; i++) {
        ...
        let getDataOption = {
            ...
            headers: {
                "User-Agent": "request",
                "Authorization" : "token 13ae28b92ccfff42b5ad7b8b94f62aa878662ed1"
            }
        };
        ...
        request(getDataOption, function (err, res, profile) {
            ...
            if (userId === data[0].login) {
                ...
                let userDataOption = {
                    url: `https://api.github.com/users/${data[0].login}?client_id=4759e5875ec96b1011ee&client_secret=fca441b57aaa2774ec1cc353721f8ee51a9094b8`,
                    headers: {
                        "User-Agent": 'request',
                        "Authorization" : "token 13ae28b92ccfff42b5ad7b8b94f62aa878662ed1"
                    }
                ...
                request(userDataOption, function (err, res, userData) {
                    ...
                    User.create(jsonUserData, function (err, result) {
                        if (err) console.log(err);
                    });
                })
                ...
                request(repositoryOptions , function(err, res, repoData){
                    if(err) console.log(err);
                    let jsonRepoData = JSON.parse(repoData);
                    console.log(jsonRepoData);
                    Repo.insertMany(jsonRepoData, function(err, result){
                        if(err) console.log(err);
                    })
                })
                if (err) {
