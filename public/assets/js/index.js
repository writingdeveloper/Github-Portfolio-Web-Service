...
(function() {
  ...
  var MODULE_MENU = (function() {
    ...
    var MOD_MENU_Button = [].slice.call(
      document.querySelectorAll(".MOD_MENU_Button")
    );
    ...
    function toggleMenu(event, el) {
      ...
      if (
        event !== null &&
        this.nextElementSibling.className === "AP_Menu_List"
      ) {
    ...
    for (
      var _iterator = Array.from(MOD_MENU_Li),
  ...
  var MODULE_ACCORDION = (function() {
    ...
    var setAutoTimer = {};
    var MODULE_ACCORDION_Panels = document.querySelectorAll(
      ".AP_accordion_panel"
    );
    //collapse all tab panels to begin with
    for (
      var _iterator2 = Array.from(MODULE_ACCORDION_Panels),
    ...
    function accordionManager(el) {
      if (el.nextElementSibling.classList.contains("AP_accordion_panel")) {
        (function() {
          ...
          if (
            parseInt(panel.style.height, 10) > 0 ||
