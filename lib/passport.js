module.exports = function (router, db, request, shortid) {
    ...
    passport.use(new GoogleStrategy({
            ...
            clientID: '455173887406-cctu6l9vuvvbu98kt4udue41u2tkp9mu.apps.googleusercontent.com',
            clientSecret: '0jFaDPCpmtnAuo936S7PeUva',
            callbackURL: 'http://127.0.0.1:3000/auth/google/callback'
        },
        function (accessToken, refreshToken, profile, cb) {
            console.log(profile);
            db.query(`SELECT 0 FROM user WHERE displayId=?`, [profile.id], function (error, data) {
                if (error) {
                    throw `Error from PassportJS GoogleStrategy : ${error}`;
                }
                // console.log(profile);
                if (data.length > 0) { // Check Registered or not
                    ...
                    db.query(`SELECT 0 FROM user WHERE loginId=?`, [profile.displayName], function (error, data) {
                        let loginId = profile.displayName;
                        let displayId = profile.id;
                        let avatarUrl = profile.photos[0].value;
                        let name = profile.displayName;
                        if (data.length > 0) {
                            console.log('Exist NickName!!');
                            db.query(`INSERT INTO user (loginId, displayId, avatarUrl, name, registerType) VALUES (?, ?, ?, ?, ?)`, [`${loginId}-${displayId}`, displayId, avatarUrl, name, 'Google']); // MySQL user Table Query
                        } else {
    ...
    passport.use(new GitHubStrategy({
            ...
            clientID: githubCredentials.local.clientID,
            clientSecret: githubCredentials.local.clientSecret,
            callbackURL: githubCredentials.local.callbackURL
        },
        function (accessToken, refreshToken, profile, cb) {
